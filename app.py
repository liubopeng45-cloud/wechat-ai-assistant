...
from utils.rate_limiter import RateLimiter      # 速率限制
from utils.message_filter import MessageFilter  # 消息过滤
...
# ---- 速率限制器 ----
# 每个用户每分钟最多 20 条消息，超过时提示"请求过频"
rate_limiter = RateLimiter(max_requests=20, window_seconds=60)

# ---- 消息过滤器 ----
# 对用户输入进行预处理（如手机号、身份证号掩码）
...
def _check_config():
    ...
    if not app.config.get('WECHAT_TOKEN'):
        missing.append('WECHAT_TOKEN')
    if missing:
        print(f'[FATAL] 缺少必要配置项: {", ".join(missing)}')
...
def health():
    ...
    返回 JSON 格式的服务状态信息，包含：
      - status: up/down
      - async_mode: 异步模式是否启用
      - ai_model: 当前使用的 AI 模型
      - total_users: 当前活跃用户数
      - timestamp: 当前时间戳

    可用于监控系统（如 uptimerobot）或容器化部署的探活检查。
    """
    return {
        'status': 'up',
        'async_mode': wechat_api.is_configured(),
        'ai_model': app.config.get('DEEPSEEK_MODEL'),
        'total_users': memory.stats.get('total_users', 0),
...
def _handle_message():
    ...
    #
    # parse_message 是 wechatpy 的核心解析函数。
    # 它接受微信标准的 XML 格式（含 CDATA 包裹的字段值），
    # 内部通过 lxml 解析 XML 树，根据 <MsgType> 标签自动
    # 映射到对应的消息子类（TextMessage、EventMessage 等）。
    # 返回的对象统一了不同消息类型的属性访问方式。
...
def _handle_text(msg):
    ...
    reset_keywords = ['重置对话', '重新开始', '清空记忆', 'clear', '/reset']
    if any(kw in raw_content for kw in reset_keywords):
        memory.clear(user_id)
        return TextReply(
            content='对话已重置，我们可以重新开始了！',
            message=msg,
        )

    # ---- 消息过滤（敏感信息掩码等） ----
    safe_content = message_filter.apply(raw_content)
    ...
    if wechat_api.is_configured():
        ...
        executor.submit(_process_and_push, user_id, safe_content, history)
    ...
    messages = [
        ...
        {"role": "user", "content": safe_content},        # 当前消息
    ]
    ...
    memory.add_message(user_id, "user", safe_content)
